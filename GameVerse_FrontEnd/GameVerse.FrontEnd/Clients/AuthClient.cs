...
namespace GameVerse.FrontEnd.Clients
{
    public class AuthClient
    {
        ...
        public async Task<AuthResponse> SignUpAsync(string username, string email, string password)
        {
            var data = new
            {
                Username = username,
            ...
            var authResponse = await response.Content.ReadFromJsonAsync<AuthResponse>()
                ?? throw new InvalidOperationException("Não foi possível desserializar a resposta de signup.");
